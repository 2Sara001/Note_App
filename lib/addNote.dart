import 'package:flutter/material.dart';
import 'homePage.dart';
import '../dp/notes_database.dart';
import 'editnote.dart';
import 'addNote.dart';
import 'dart:async';

class addNote extends StatefulWidget {
  const addNote({Key? key}) : super(key: key);

  @override
  State<addNote> createState() => _NewNoteState();
}

class _NewNoteState extends State<addNote> {
  NoteDatabase sqlDb = NoteDatabase();
  TextEditingController title = TextEditingController();
  TextEditingController description = TextEditingController();
  GlobalKey<FormState> key = GlobalKey<FormState>();
  int color = 0;
  final colors = [
    const Color(0xff1321E0),
    const Color(0xffFFFFFF),
    const Color(0xffF28B81),
    const Color(0xffF7BD02),
    const Color(0xffFBF476),
    const Color(0xffCDFF90),
    const Color(0xffA7FEEB),
    const Color(0xffCBF0F8),
    const Color(0xffAFCBFA),
    const Color(0xffD7AEFC),
    const Color(0xffFBCFE9),
    const Color(0xffE6C9A9),
    const Color(0xffE9EAEE),
  ];

  @override
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        backgroundColor: color == 0
            ? Color(0xff1321E0)
            : color == 1
                ? Color(0xffFFFFFF)
                : color == 2
                    ? Color(0xffF28B81)
                    : color == 3
                        ? Color(0xffF7BD02)
                        : color == 4
                            ? Color(0xffFBF476)
                            : color == 5
                                ? Color(0xffCDFF90)
                                : color == 6
                                    ? Color(0xffA7FEEB)
                                    : color == 7
                                        ? Color(0xffE6C9A9)
                                        : color == 8
                                            ? Color(0xffE9EAEE)
                                            : Color(0xffD7AEFC),
        title: const Text('New Note'),
        actions: [
          IconButton(
              onPressed: () {
                buildbottomsheet(context);
              },
              icon: const Icon(Icons.more_vert)),
          IconButton(
              onPressed: () async {
                int response = await sqlDb.insertNote('''
                          INSERT INTO notes ( title , description , color  )
                         VALUES("${title.text}","${description.text}" , "${color}")
                         ''');

                if (response > 0) {
                  Navigator.of(context).pop();
                }
              },
              icon: const Icon(Icons.done_rounded)),
        ],
      ),
      body: Container(
        margin: EdgeInsets.only(top: 5),
        child: Form(
          key: key,
          child: Column(
            children: [
              Container(
                margin: const EdgeInsets.symmetric(horizontal: 30, vertical: 20),
                child: TextFormField(
                  controller: title,
                  decoration: const InputDecoration(hintText: 'Type Something...', hintStyle: TextStyle(fontSize: 22, color: Color.fromARGB(255, 19, 33, 224))),
                ),
              ),
              Container(
                margin: const EdgeInsets.symmetric(horizontal: 30),
                child: TextFormField(
                  controller: description,
                  decoration: const InputDecoration(hintText: 'Type Something...', hintStyle: TextStyle(fontSize: 18, color: Colors.grey), border: InputBorder.none),
                ),
              ),
            ],
          ),
        ),
      ),
    );
  }

  buildbottomsheet(BuildContext context) async {
    showModalBottomSheet(
        context: context,
        builder: (_) {
          return Container(
            color: Color.fromARGB(255, 19, 33, 224),
            margin: EdgeInsets.fromLTRB(0, 0, 0, 0),
            child: Container(
              margin: EdgeInsets.fromLTRB(20, 50, 0, 0),
              child: Column(
                children: [
                  Row1(
                    'Share with your frinds',
                    const Icon(
                      Icons.share,
                      color: Colors.black,
                    ),
                  ),
                  const SizedBox(
                    height: 20,
                  ),
                  Row(
                    children: [
                      Container(
                        margin: EdgeInsets.only(right: 10),
                        child: CircleAvatar(
                          backgroundColor: Colors.grey,
                          radius: 22,
                          child: IconButton(
                              onPressed: () {},
                              icon: Icon(
                                Icons.delete,
                                color: Colors.black,
                              )),
                        ),
                      ),
                      Text(
                        'Delete',
                        style: TextStyle(color: Colors.white),
                      )
                    ],
                  ),
                  Row1(
                    'Duplicade',
                    Icon(
                      Icons.copy_rounded,
                      color: Colors.black,
                    ),
                  ),
                  SizedBox(
                    height: 10,
                  ),
                  _colorPalette()
                  // Row2(),
                ],
              ),
            ),
          );
        });
  }

  Container Row1(String t, Icon i) {
    return Container(
      margin: const EdgeInsets.only(top: 20),
      child: Row(
        children: [
          Container(
            margin: const EdgeInsets.only(right: 10),
            child: CircleAvatar(
              backgroundColor: Colors.grey,
              radius: 22,
              child: i,
            ),
          ),
          Text(
            t,
            style: const TextStyle(color: Colors.white),
          )
        ],
      ),
    );
  }

  Widget _colorPalette() {
    return SingleChildScrollView(
      scrollDirection: Axis.horizontal,
      child: Wrap(
          children: List<Widget>.generate(
        7,
        (index) => InkWell(
          onTap: () {
            setState(() {
              color = index;
            });
            Navigator.of(context).pop();
          },
          child: Padding(
            padding: const EdgeInsets.only(right: 8),
            child: CircleAvatar(
              radius: 23,
              backgroundColor: Colors.grey,
              child: CircleAvatar(
                child: color == index
                    ? Icon(
                        Icons.done,
                        size: 16,
                        color: Colors.black,
                      )
                    : null,
                backgroundColor: index == 0
                    ? Color(0xff1321E0)
                    : index == 1
                        ? Color(0xffFFFFFF)
                        : index == 2
                            ? Color(0xffF28B81)
                            : index == 3
                                ? Color(0xffF7BD02)
                                : index == 4
                                    ? Color(0xffFBF476)
                                    : index == 5
                                        ? Color(0xffCDFF90)
                                        : index == 6
                                            ? Color(0xffA7FEEB)
                                            : index == 7
                                                ? Color(0xffE6C9A9)
                                                : index == 8
                                                    ? Color(0xffE9EAEE)
                                                    : Color(0xffD7AEFC),
                radius: 22,
              ),
            ),
          ),
        ),
      )),
    );
  }
}
